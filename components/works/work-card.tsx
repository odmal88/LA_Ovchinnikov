...
export function WorkCard({ work, className, priority = false }: WorkCardProps) {
  ...
  return (
    ...
    >
      ...
      {/* Информация */}
      <div className="p-4">
        <h3 className="font-serif text-base md:text-lg text-foreground line-clamp-2 mb-2 group-hover:text-olive transition-colors">
          {work.title}
        </h3>

        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
          {work.date && <span>{work.date}</span>}
          {!work.date && work.period && (
            <span>{periodLabels[work.period]}</span>
          )}
